...
public class AggregateDecisionByTradeDatetime extends Operator {
	 ...
	 * Constructor
	 */
	public AggregateDecisionByTradeDatetime(OperatorDescription description) {
	...
	public void doWork() throws OperatorException {
		...
		PredictionCount.minorityThreshold=getParameterAsInt(PARAMETER_MINORITY_THRESHOLD);

		PredictionCount.majorityThreshold=getParameterAsInt(PARAMETER_MAJORITY_THRESHOLD);

		PredictionCount.minorityBooleanOperator=getParameterAsInt(PARAMETER_MINORITY_BOOLEAN_OPERATOR);

		PredictionCount.majorityBooleanOperator=getParameterAsInt(PARAMETER_MAJORITY_BOOLEAN_OPERATOR);
		
		PredictionCount.conjunctionOperator1=getParameterAsInt(PARAMETER_CONJUNCTION_BOOLEAN_OPERATOR_1);

		
		PredictionCount.usePercent = getParameterAsBoolean(PARAMETER_USE_PERCENTS);
		
		
		
		PredictionCount.minorityPercentThreshold = (float)getParameterAsDouble(PARAMETER_MINORITY_THRESHOLD_PERCENT);
		PredictionCount.majorityPercentThreshold = (float)getParameterAsDouble(PARAMETER_MAJORITY_THRESHOLD_PERCENT);
		
		
		setParameter(PARAMETERS_TO_STRING,PredictionCount.toStringThreshold());
		PredictionCount.hasPositivePredictions=false;
		...
		if (predictionAttribute.getMapping().getIndex("0")!=predictionAttribute.getMapping().getIndex("0"))
		{
			...
			int tmp =PredictionCount.label0; 
			PredictionCount.label0=PredictionCount.label1;
			PredictionCount.label1=tmp;
		...
		List <Integer> toRemove = new ArrayList<Integer>();
			...
			Map<Long, PredictionCount> predictionCounts=aggregatePredictionByDateTime(inputSet);
			...
			for (int i=0;i<inputSet.size();++i)
			{
				...
				PredictionCount count = predictionCounts.get(id);
				int newPrediction=predictionCounts.get(id).getAssemblyPrediction();
				example.setPredictedLabel(newPrediction);
				
				if (!alreadyKeptDateTime.contains(count.datetime)) {
					alreadyKeptDateTime.add(count.datetime);
				}
				else
				{
					toRemove.add(i);
			...
			//if (!PredictionCount.hasPositivePredictions || !PredictionCount.hasNegativePredictions)
...
*/
	private Map<Long, PredictionCount> aggregatePredictionByDateTime(ExampleSet inputSet) throws SQLException
	{	
		Map <String,PredictionCount > countByDateTime = new HashMap<String,PredictionCount>();
		
		Map  <Long, String > dateTimesById = getDateTimesById(inputSet);
		
		
		
		Map <Long, PredictionCount> predictionCountById= new HashMap<Long,PredictionCount>();
		...
		if (verbose) {
			System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			System.out.println(PredictionCount.toStringThreshold());
			System.out.println("datetime\tcount 0\tcount 2\t%\tdec\tlbl");
		}
		Set <String>keys = countByDateTime.keySet();
		for(String key:keys)
		{
			PredictionCount pc= countByDateTime.get(key);
			if (verbose)
