...
public class KNNUndersamplingNoLabelKernel extends Operator {
	 ...
	 * Constructor
	 */
	public KNNUndersamplingNoLabelKernel(OperatorDescription description) {
		super(description);
	...
	protected List<Integer> obtainInstancesToRemove(ExampleSet test, String majorityClassLabel, int k) throws Exception {
		...
		for (int i = 0; i < test.size(); i++) {
			...
			if (decideToRemove(neighbours, majorityClassIndex, threshold, example,i, values)) {
				toRemove.add(i);
			}
		}
	...
	protected boolean decideToRemove(Collection<Tupel<Double, Integer>> neighbours, int majorityClass, int threshold, Example example,int i, double[] exampleValues ) {
		...
		if (useDistanceWeight) {
			//values = ((LinearListFixed)knn).getLatestValues();
		...
		while (iterator.hasNext()) {
			...
			if (useDistanceWeight) {
				//The distance is adjusted using kernel
				double kernelDistance =   neighbor.getFirst();  //kernel.calculateDistance(values[j],exampleValues);
				double weight = kernelDistance/maxDistance;
		...
		boolean removeIt = (numberFromMinorityClasses >= threshold);
	...
	private GeometricDataCollection obtainNeighbors(ExampleSet exampleSet) throws Exception {
		...
		if (labelAtt.isNominal()) {
			// classification
			GeometricDataCollection<Integer> samples = new LinearListKernel(kernel);
