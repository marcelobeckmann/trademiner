...
public class RestrictiveAlignment2MPercent extends SimpleAlignment {
	...
	private   double percentThreshold=0.60;
	...
	//	if (algs.size()>1)
		...
		for (Alignment al : algs) {
			...
			//--------------------------------------------------------------
			if (al.percent_change>=percentThreshold)
			{
				al.label = 2;
			}
			else if (al.percent_change<=-percentThreshold)
			{
				al.label = -2;
			}
			else
	...
	/**
	 * @param percentThreshold the percentThreshold to set
	 */
